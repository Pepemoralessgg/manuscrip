...
/**
 * manuscripts.user_id suele tener FK a profiles(id). Auth no crea la fila en profiles salvo trigger;
 * sin fila, el insert en manuscripts falla (23503).
 */
async function ensureProfileForUser(userId) {
  ...
  try {
    ...
    if (!adminErr && adminData?.user?.email) {
  ...
  const candidates = [
    { id: userId },
    email ? { id: userId, email } : null,
    {
      id: userId,
      email: email || `user-${userId}@placeholder.invalid`,
